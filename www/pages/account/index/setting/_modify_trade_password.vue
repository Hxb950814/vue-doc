...
<template>
    <div class="modify-form">
        <div>
            <el-input v-model.trim="oldpass" type="password" value="" placeholder="请输入原密码"
                      maxlength="6" style="width: 250px"
            />
        </div>
        <div>
...
<script lang="ts">
    ...
    export default class ModifyTradePassword extends Vue {
        ...
        async sureModifyInfo() {
            ...
            if (!isInteger(this.oldpass)) {
                this.$showToast('原密码必须是数字格式');
                return;
            }
            if (!this.newpass) {
            ...
            this.$showLoading();
            const [err, resp] = await postAsync('/systemInstall/updatePassword', {

...
/* sleep函数 */
// export function waitFoTime(time: number) {
...
export function spliceStr(value:string, num:number) {
    const str = value.slice(0, num) + value.slice(8, 12).replace(/[0-9\u4E00-\u9FA5]/g, '*') + value.slice(-num);
    return str;
}
/* 分享的函数, 在客户端使用 */

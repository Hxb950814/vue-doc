...
<script lang="ts">
    ...
    export default class extends Vue {
        ...
        async getDetail() {
            // 取得新股详情
            this.$showLoading();
            const [err, resp] = await this.$postAsync('/newStock/queryNewStockInfoById', {
                newStockId: this.id
            });

            this.$closeLoading();
            if (resp?.code === '000000') {
                this.editData.newStockName = resp?.data?.newStockName; // 新股名称
                this.editData.applyCode = resp?.data?.applyCode; // 申购代码
                this.editData.stockType = resp?.data?.stockType; // 股票类型
                this.editData.issueNum = resp?.data?.issueNum; // 发行总量
                this.editData.issuePrice = resp?.data?.issuePrice; // 发行价格
                this.editData.pERatio = resp?.data?.pERatio; // 市盈率
                this.editData.netIssueNum = resp?.data?.netIssueNum; // 网上发行总量
                this.editData.netApplyLimit = resp?.data?.netApplyLimit; // 网上申购上限
                this.editData.applyDate = resp?.data?.applyDate; // 申购日期
                this.editData.allotmentNumberDate = resp?.data?.allotmentNumberDate;// 配号日期
                this.editData.winResultDate = resp?.data?.winResultDate; // 中签结果日期
                this.editData.listDate = resp?.data?.listDate; // 上市日期
                this.editData.paymentBySign = resp?.data?.paymentBySign; // 一签缴款
                this.editData.companyIntroduce = resp?.data?.companyIntroduce; // 公司介绍
            } else {
                this.$showToast(resp?.message || '网络异常');
            }
        }

    ...
    <div>
        ...
        <div class="form-input-area">
            <el-form ref="dataForm" class="form-data form-style" :model="editData" :rules="dataRule">
                ...
                <el-form-item :label="$route.query.title + '位置：'" prop="positionId" label-width="120px">
                    <el-select
...
<script lang="ts">
    ...
    export default class extends Vue {
        ...
        dataRule: any = {
            ...
            channel: [{required: true, message: '至少选择一个渠道', trigger: 'change'}],
        ...
        async getDetail() {
            const [err, resp] = await this.$postAsync('/icon/geticonDetail', {iconId: this.id}, {
                paramType: 'form'
            });
            if (resp?.code === '000000') {
                this.editData.name = resp?.data.imageTitle;
                this.editData.type = resp?.data.type;
                this.editData.positionId = String(resp?.data.positionId);
                this.editData.channel = resp?.data.displayPlatform.split(',');
                this.editData.power = String(resp?.data.power);
                this.editData.status = String(resp?.data.display);
                this.editData.image = resp?.data.imageUrl;
                this.editData.order = resp?.data.sortby;
                this.editData.linkType = String(resp?.data.urlType);
                this.editData.link = resp?.data.url;
                this.editData.linkPageId = String(resp?.data.pageId);
                this.editData.linkKey = resp?.data.pageKey;
                this.editData.linkValue = resp?.data.pageValue;
        ...
        async doSave() {
            ...
            const data: any = {
                ...
                positionId: this.editData.positionId,
                ...
                positionName: this.getPositionStr(), // 平台

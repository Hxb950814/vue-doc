...
/* 单个时间限制 */
export function makeDatePickerLimit(nowTime = new Date(), isLater = true /* 是否只允许大于前面的时间 */) {
    return (time: Date) => {
        if (isLater) {
...
/* 2或4个时间联动 */
export function makeDatePickerRelationLimit(dataObj: any, keysArr: Array<string>, options?: any) {
...
export function checkPermissionNotExist(permissionStr: string) {
    ...
    const permission = store.getters.permissionList;
    for (const str of permissionArr) {
        if (permission.includes(str)) { // 存在一个就return false
            // console.log(1111);
            return false;
        }
    }

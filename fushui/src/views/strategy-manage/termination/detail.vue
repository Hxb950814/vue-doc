<template>
    <div>
        <div class="page-cap-title page-cap-titleH">
            <span>策略运行-【{{ pageData && pageData.strategyName }}】-【{{ pageData && pageData.mobile }}】-【{{ pageData && pageData.futureCompanyName }}】-【{{ pageData && pageData.futureAccount }}】</span>
        </div>
        <!--    内容部分    -->
...
<script lang="ts">
    ...
    import statisticsView from '../running/compontent/statistics.vue';
    ...
    export default class extends Vue {
        ...
        async getDetail() {
            // 通过策略id取得查看的数据
            this.$showLoading();
            const [err, resp] = await this.$postAsync('/strategy/queryStrategyExecuteStatusInfo', {
            ...
            if (resp?.code === '000000') {
                ...
                const {
                    ...
                    strategyPosition, strategyPositionList, tradeStatus, orderStatus, strategyProfitChartConfigList, strategyCapital, totalProfitMoney, totalProfitRate, tradeLevel
                } = resp?.data;

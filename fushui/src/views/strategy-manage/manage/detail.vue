<template>
    <div>
        <div class="page-cap-title page-cap-titleH">
            <span>查看策略</span>
            <div>
                <el-button v-if="canEdit" class="page-table-btn" @click="cancelEdit">
                    取消编辑
                </el-button>
                <el-button type="primary" class="page-table-btn" @click="editInfo">
                    {{ canEditText }}
                </el-button>
            </div>
        </div>
        ...
        <div class="hbb-contentBox">
                    ...
                    <div class="group">
                        ...
                        <div class="inputBox radioBox">
                            <div v-viewer class="imgBox">
                                <my-image-upload :disabled="!canEdit" :url.sync="pageData.strategyIntroduceMobile" />
                                <p style="text-align: center">
                                    移动端
                                </p>
                            </div>
                            <div v-viewer class="imgBox">
                                <my-image-upload :disabled="!canEdit" :url.sync="pageData.strategyIntroducePc" />
                                <p style="text-align: center">
                                    PC端
                                </p>
                            </div>
            ...
            <div class="right-box">
                ...
                <div class="hbb-ownBox">
                    <div class="title">
                        配置合约 <span v-if="canEdit" @click="addContractClick">+</span>
                    </div>
                    <el-table
                    ...
                    >
                        ...
                        >
                            <template slot-scope="scope">
                                <span style="cursor: pointer;color: #1a7ef8" @click="delClick(scope.row)">
                                    {{ canEdit ? '移除' : '-' }}
                                </span>
                            </template>
                        </el-table-column>
...
<script lang="ts">
    ...
    export default class extends Vue {
        canEdit:boolean = false; // 是否是编辑模式
        canEditText:string = '编辑'; // canEdit:false '编辑'； true：'保存'
        ...
        isCheckAll:boolean = false; // 当前选中的按钮 是否选择全部
        ...
        pageData = {
            ...
            defaultRate: '', // 违约比例
            fundTopLimit: '', // 资金上限
            isHandClose: '', // 手动关闭 0=不支持，1=支持
            isRecommend: '', // 是否推荐 0=不推荐，1=可推荐
            operator: '', // 操作人
            profitRateList: [], // 止盈止损率List
            serviceRate: '', // 服务费率
            status: '', // 当前状态 0=不展示，1=展示
            ...
            strategyName: '', // 策略名称
            strategyPosition: '', // 用户持仓 0=不显示，1=显示
            strategyProfitChartConfigList: [], // 收益图表配置list
            useClient: '', // 使用前端 0=移动端 1=pc端(多选）
            validDay: '', // 有效天数
            serviceType: '', // 费用模式 0=比例模式、1=订阅模式
            serviceBean: '', // 订阅模式k豆数量
            strategyQuotaType: '', // 策略额度 0=无限制,1=固定额度
            strategyQuota: '', // 策略额度
            assetStepSize: '', // 资产步长(万) 输入数字为0.1-1 之间，小数点后一位
            limitNumType: '', // 限制次数类型 0=无限制， 1=固定次数
            limitNum: '', // 限制次数
            newUserUse: '' // 新户专享 0=否，1=是
        }; // 页面数据源
        ...
        async getDetail() {
            // 通过策略id取得查看的数据
            this.$showLoading();
            const [err, resp] = await this.$postAsync('/strategy/queryStrategyInfo', {
            ...
            if (resp?.code === '000000') {
                const {
dealLog, defaultRate, fundTopLimit, isHandClose, isRecommend, operator, profitRateList, serviceRate, status,
                    strategyContractConfigDoList, strategyId, strategyIdString, strategyIntroduceMobile, strategyIntroducePc,
                    strategyLabel1, strategyLabel2, strategyLabel3, strategyName, strategyPosition, strategyProfitChartConfigList, useClient,
                    validDay, serviceType, serviceBean, strategyCapital, strategyQuotaType, strategyQuota, assetStepSize, limitNumType, limitNum, newUserUse
} = resp?.data;
                this.pageData.dealLog = String(dealLog);
                this.pageData.defaultRate = defaultRate;
                this.pageData.fundTopLimit = fundTopLimit;
                this.pageData.isHandClose = String(isHandClose);
                this.pageData.isRecommend = String(isRecommend);
                this.pageData.operator = operator;
                this.pageData.profitRateList = profitRateList;
                this.pageData.serviceRate = serviceRate;
                this.pageData.status = String(status);
                this.pageData.strategyContractConfigDoList = strategyContractConfigDoList;
                ...
                this.pageData.strategyPosition = String(strategyPosition);
                ...
                this.pageData.strategyQuotaType = String(strategyQuotaType);
                this.pageData.strategyQuota = strategyQuota;
                this.pageData.assetStepSize = assetStepSize;
                this.pageData.limitNumType = String(limitNumType);
                this.pageData.limitNum = limitNum;
                this.pageData.newUserUse = String(newUserUse);
                this.pageData.strategyCapital = strategyCapital;
                // 重组盈亏数据
                this.surplusList = this.pageData.profitRateList.filter((y:any) => y.profitType === 0);
                this.damageList = this.pageData.profitRateList.filter((x:any) => x.profitType === 1);
                this.surplusList = this.surplusList.map((x:any, i:number) => {
                    return {
                        profitType: 0,
                        profitRate: x.profitRate,
                        strategyProfitId: x.strategyProfitId,
                ...
                this.damageList = this.damageList.map((y:any, i:number) => {
                    return {
                        ...
                        strategyProfitId: y.strategyProfitId,
                        placeholder: '最大止损-' + (i + 1)
                    };
                });
                // 重组配置图标按钮收益数据
                if (this.pageData.strategyProfitChartConfigList.length > 0) {
                    if (this.pageData.strategyProfitChartConfigList.length === 8) {
                        this.isCheckAll = true;
                    } else {
                        this.isCheckAll = false;
                    }
                    this.navList.forEach((x:any) => {
                        (this.pageData.strategyProfitChartConfigList as any).filter((y:any) => {
                ...
                this.pageData.useClient = useClient.replace(',', '').split('');
                this.pageData.validDay = validDay;
                // 查询模拟数据
        ...
        editInfo() {
            // 点击添加效果
            if (!this.canEdit) {
                this.canEditText = this.canEdit ? '编辑' : '保存';
                this.canEdit = true;
                return;
            }

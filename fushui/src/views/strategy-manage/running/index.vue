        ...
        <div class="page-cap-content">
            <div class="hbb-page-top">
                <el-form :inline="true" :model="search" class="demo-form-inline">
                    ...
                    <el-form-item label="订单时间" class="hbb">
                        <el-date-picker
            ...
            <div class="hbb-page-table">
                ...
                >
                    ...
                    >
                        <template slot-scope="scope">
                            <span style="cursor: pointer;color: #1a7ef8" v-permission="'08020101'" @click="lookDetail(scope.row)">查看</span>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
...
<script lang="ts">
    ...
    export default class extends Mixins(commonTable) {
        time:string = ''; // 选择时间的占位
        search: any = {
            executeStatus: '0', // 策略列表 0=执行策略，1=终止策略
            endTime: '', // 结束时间
            startTime: '', // 开始时间
            status: '0', // 终止方式 0=运行中，1=止损，2=止盈，3=到期，4=手动
            user: '', // 订阅用户
            strategyName: '' // 策略名称
        };
            ...
            if (resp?.code === '000000') {
                ...
                resp.data.result.forEach((item: any, idx: number) => {
                    list.push({
                        ...
                        operateFund: item.operateFund, // 操作资金
                        lossRate: item.lossRate, // 最大止损
                        profitRate: item.profitRate, // 最大止盈
                        validTime: item.validTime, // 有效期至
                        frozenBean: item.frozenBean, // 冻结K豆
                        mobile: item.mobile, // 订阅用户
                        futureCompany: item.futureCompany, // 期货公司
                        futureAccountNumber: item.futureAccountNumber, // 期货账号
                        dealCount: item.dealCount, // 交易次数
                        profitLossDrop: item.profitLossDrop, // 最大回撤
                        profitLossTotal: item.profitLossTotal // 最终盈亏
                    });
                });

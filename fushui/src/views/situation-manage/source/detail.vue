...
<script lang="ts">
    ...
    export default class extends Vue {
        ...
        async getData() {
            const [_, resp] = await this.$getAsync('/quotation/getQuotationSourceDetail', {
                quotationSourceId: this.id
            });
            if (resp?.code === '000000') {
                this.editData.name = resp.data.sourceName;
                this.editData.exchangeName = resp.data.fullName;
                this.editData.status = resp.data.status + '';
                this.editData.order = resp.data.sortby;
                this.editData.contractNumber = resp.data.contractNumber;
                this.editData.speciesNumber = resp.data.speciesNumber;
                if (Number(resp.data.contractNumber) === 0 && Number(resp.data.speciesNumber) === 0) {

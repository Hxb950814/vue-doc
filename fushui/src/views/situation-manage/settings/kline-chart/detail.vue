...
<script lang="ts">
    ...
    export default class extends Mixins(indexSettingMixins) {
        ...
        async getData() {
            const [err, resp] = await this.$getAsync('/chart/getChartDetail', {
                chartId: this.id
            });
            const list: any[] = [];
            if (resp?.code === '000000') {
                this.editData.name = resp.data.chartName;
                this.editData.type = resp.data.chartTypeId + '';
                this.editData.order = resp.data.sortby;
                this.editData.status = resp.data.status + '';
                this.setChartKindById(this.editData.type);
                this.setChartIndexData(resp.data.chartIndicatorsVoList || []);
            }
        }

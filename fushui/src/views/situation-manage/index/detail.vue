                ...
                <el-form-item label="指数数据：" prop="" label-width="120px">
                    <div class="price-label">
                        <div class="tab">
                            ...
                            <div
                                class="tab-value"
                                :class="['tab-value', upDowns === '' ? '' : parseFloat(upDowns) > 0 ? 'up' : 'down']"
                            >
                                {{ price === '' ? '--' : price }}
                            </div>
                        </div>
...
<script lang="ts">
    ...
    export default class extends Vue {
        ...
        isFirstInitQuotationContractId = false;
        editData: any = {
            name: '', // 指数名称
            apiCode: '', // apiCode
        ...
        onSourceChange() {
            if (!this.isFirstInitQuotationSpeciesId) { // 如果不是第一次出
        ...
        onSpeciesChange() {
            if (!this.isFirstInitQuotationContractId) { // 如果不是第一次出
                this.editData.quotationContractId = ''; // 先清空
            }
            this.isFirstInitQuotationContractId = false;
        ...
        async getData() {
            this.$showLoading();
            const [_, resp] = await this.$getAsync('/quotation/getQuotationIndexDetail', {
                quotationIndexId: this.id
            });
            this.$closeLoading();
            if (resp?.code === '000000') {
                this.editData.name = resp.data.indexName;
                this.editData.quotationSourceId = resp.data.quotationSourceId + '';
                this.editData.quotationSpeciesId = resp.data.quotationSpeciesId + '';
                this.editData.quotationContractId = resp.data.quotationContractId + '';
                //
                this.editData.sortby = Number(resp.data.sortby);
                this.editData.status = String(resp.data.status);

                this.editData.dataIdentity = resp.data.dataIdentity;
                this.editData.apiCode = resp.data.apiCode;
                this.editData.dataSource = resp.data.dataSource + '';
                this.isFirstInitQuotationSpeciesId = true;
                this.isFirstInitQuotationContractId = true;

                this.oldApi = this.editData.apiCode;
                if (dataSocketInstance && this.editData.apiCode) {
                    dataSocketInstance.sendContractTickMessage(this.editData.apiCode);
                }
            }

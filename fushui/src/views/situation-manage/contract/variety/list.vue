        ...
        <div class="nav-common-search">
            <div class="search-line">
                ...
                <div class="search-union right">
                    ...
                    <el-button v-permission="'06030301'" type="primary" @click="clickAdd">
                ...
                <el-table-column prop="situation" label="行情数据" width="220px">
                    ...
                    <template slot-scope="scope">
                        <div class="title-flex" :class="scope.row.changeValue >= 0 ? 'up' : 'down'">
                            ...
                            <span>{{ Number(scope.row.changeValue).toFixed(2) }}</span>
                            <span>{{ scope.row.changeRate }}</span>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column prop="mainContractName" label="当前主力合约">
                </el-table-column>
            ...
            <table class="position-table">
                <tr>
                    ...
                    <td :class="tickData.changeValue >= 0 ? 'up' : 'down'">
                        {{ Number(tickData.price).toFixed(2) }}
                    </td>
                    <td>涨跌</td>
                    <td :class="tickData.changeValue >= 0 ? 'up' : 'down'">
                        {{ Number(tickData.changeValue).toFixed(2) + '/' }}
                        {{ tickData.changeRate + '%' }}
                    </td>
                </tr>
                <tr>
...
<script lang="ts">
    ...
    export default class extends Mixins(commonTable) {
            ...
            if (resp?.code === '000000') {
                ...
                (resp?.data?.result || []).forEach((item: any, idx: number) => {
                    list.push({
                        ...
                        mainContractName: item.mainContractName,
                        price: '--',
                        changeValue: '--',
                        changeRate: '--'
                    });
        ...
        clickAdd(id: string) {
            this.$router.push({
                path: '/situation_manage/contract/variety/add',
                query: {
                    id
                }
            });
        ...
        clickEdit(id: string) {
            this.$router.push({
                path: '/situation_manage/contract/variety/detail',
                query: {
                    id
                }
            });

...
<!--                    </el-table-column>-->
                    ...
                    >
                        <template slot-scope="scope">
                            <span v-permission="'07050201'" style="color:#3D91F7;cursor: pointer" @click="lookDetail(scope.row)">详情</span>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
        ...
        <!--   查看详情     -->
        <el-dialog v-if="detailVisible" :visible.sync="detailVisible">
...
<script lang="ts">
    ...
    export default class extends Mixins(commonTable) {
        search = {};
        detailVisible:boolean = false; // 查看详情弹层是否启动
        ...
        clickDownload() {
            ...
            this.search = (this.$refs.SearchView as any).search;
            const search = (this.search as any);
            this.$post('/userOrder/strikecouldOrderListExcel', {
                orderNo: search.orderNo,
                endTime: search.endTime,
                startTime: search.startTime,
                code: search.code,
        ...
        lookDetail(row:any) {
            ...
            setTimeout(() => {
                (this.$refs.DetailModal as any).active(row);
            }, 0);
        }
    }
</script>

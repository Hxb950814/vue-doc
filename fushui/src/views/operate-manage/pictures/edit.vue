...
<script lang="ts">
    ...
    export default class extends Vue {
        ...
        async getData() {
            this.$showLoading();
            const [err, resp] = await this.$postAsync('/banner/getBannerDetail', {
                bannerId: this.id
            }, {
                paramType: 'form'
            });
            ...
            if (resp?.code === '000000') {
                ...
                this.editData.power = data.power + ''; // 权限

                this.editData.status = String(data.display ? 1 : 0); // 状态
                this.editData.image = data.imageUrl; // 图片地址
                this.editData.order = data.sortby; // 顺序
                this.editData.publishTimeStart = data.startTime || ''; // 起始时间
                this.editData.publishTimeEnd = data.endTime || ''; // 结束时间

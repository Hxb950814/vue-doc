...
<script lang="ts">
    ...
    export default class extends Mixins(leaveCheck) {
        ...
        async getData() {
            // 获得数据
            this.$showLoading();
            const [err, resp] = await this.$postAsync('/icon/getpageDeatil', {
                pageId: this.id
            }, {
                paramType: 'form'
            });
            ...
            this.$closeLoading();
            if (resp?.code === '000000') {
                this.editData.name = resp.data.pageName; // 内容
                this.editData.id = resp.data.pageValue; // 标题
                this.editData.keyName = resp.data.pageKey || ''; // 位置
                this.editData.status = resp.data.display ? '1' : '0'; // 位置
            } else {

...
<script lang="ts">
    ...
    export default class extends Vue {
        ...
        mounted() {
            this.popupId = String(this.$route.query.id);
            this.getDetail();
        ...
        async getDetail() {
            this.$showLoading();
            const [err, resp] = await this.$postAsync('/popup/queryPopupInfoById', {popupId: this.popupId});
            if (resp?.code === '000000') {
                const data = resp?.data;
                this.editData.isLogin = String(data.isLogin);
                this.editData.popupFrequency = String(data.popupFrequency);
                this.editData.popupPosition = String(data.popupPosition);
                this.editData.skipLink = String(data.skipLink);
                if (this.editData.skipLink === '1') { // 内部链接
                    // data.pageKey = this.editData.linkKey;
                    // data.pageValue = this.editData.linkValue;
                    this.editData.linkPageId = data.skipLinkUrl;
                } else if (this.editData.skipLink === '2') {
                    this.editData.skipLinkUrl = data.skipLinkUrl;
                } else if (this.editData.skipLink === '0') {
                    this.editData.skipLinkUrl = '';
                }
                this.editData.linkPageId = String(data.skipLinkUrl);
                this.editData.publishTimeStart = String(data.startTime);
                this.editData.publishTimeEnd = String(data.endTime);
                this.editData.status = String(data.status);
                this.editData.popupImageUrl = String(data.popupImageUrl);
                this.editData.popupName = String(data.popupName);
            }
            this.$closeLoading();
        }
        ...
        async doSave() {
            ...
            if (this.editData.skipLink === '1') { // 内部链接
                ...
                data.skipLinkUrl = this.editData.skipLinkUrl;
            } else if (this.editData.skipLink === '0') {
                data.skipLinkUrl = '';
            }
            this.$closeLoading();
            ...
            if (resp?.code === '000000') {
                ...
                this.$showToast(resp?.message || '编辑失败');
            }
        }

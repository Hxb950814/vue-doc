...
<!--            <span>&lt;</span>-->
        ...
        <div class="hbb-curriculum-bottom hbb-listen-detail-bottom">
            <van-tabs v-model="currentTab" @click="tabClick">
                <span class="hbb-look-top" v-if="pageData.hasBuy !== 1 && pageData.courseTip">{{pageData.courseTip}}</span>
                <van-tab title="课程介绍">
                    <AudioIntroductionView :srcImg="pageData.courseDescribeUrl"></AudioIntroductionView>
                </van-tab>
                <van-tab title="课程目录">
                    <CourseCatalogView :hasBuy="pageData.hasBuy" :type="1" :list="pageData.courseChapterDoList"></CourseCatalogView>
                </van-tab>
            </van-tabs>
        </div>
        ...
        <div class="hbb-buy-footer">
            <template v-if="pageData.display === 2">
                <template v-if="!pageData.hasBuy">
                    ...
                    <div class="b" v-if="pageData.isLimitTimeDiscount === 1 && pageData.discountTimeType === 1">
                        <h2>{{time_text}}</h2>
                        <p>距离优惠结束</p>
                    </div>
                    <div class="c">
            ...
            <template v-if="pageData.display === 1">
                <div class="a">
                    <h2>¥ {{(pageData.isLimitTimeDiscount === 1 ? pageData.discountPrice : pageData.markPrice) | money }}</h2>
                    <s v-if="pageData.isLimitTimeDiscount === 1">¥{{pageData.markPrice | money}}</s>
                </div>
                ...
                <div class="c">
                   <van-button color="#999">敬请期待</van-button>
                </div>
            </template>
...
<script lang="ts">
    ...
    import {funcUrlDel, tabIndexFn} from "@/utils";
    ...
    export default class extends Vue {
        ...
        mounted() {
            ...
            this.userId = Number(this.$route.query.recommendUserId);
            this.type = String(this.$route.query.type);
            this.t = String(this.$route.query.t) || '0';
            if(this.t === '0') {
                this.currentTab = 0;
            } else {
                this.currentTab = 1;
        ...
        getQueryNoListenCourseInfo() {
            ...
            HomeModule.getQueryNoListenCourseInfo(requestObj).then((res:any) => {
                ...
                this.pageData.display = res?.data.display || 0;
                ...
                this.countTime =  this.pageData.discountEndTime ? (new Date(dayjs(this.pageData.discountEndTime).format('YYYY/MM/DD HH:mm:ss')).getTime() -  new Date().getTime())/1000 : this.pageData.discountEndTime
                // 处理体验问题
                this.$nextTick(() => {
